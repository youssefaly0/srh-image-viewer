...
class EliomoduleImageModule(IImageModule):
    def __init__(self):
        super().__init__()
    ...
    def process_image(self, image_data: np.ndarray, metadata: dict, params: dict) -> np.ndarray:
        ...
        if operation == "Gaussian Blur":
            ...
            if input_float.ndim == 3 and input_float.shape[2] in [3, 4]:
                   channels = []
            for i in range(3):
                channel = input_float[:, :, i]
                equalized = skimage.exposure.equalize_hist(channel)
                channels.append(equalized)

                processed_data = np.stack(channels, axis=-1)
            else:
                processed_data = skimage.exposure.equalize_hist(input_float)

                processed_data = processed_data * np.max(image_data)
